...
const StudentListComponent = () => {
    ...
    const sortStudents = (students: Student[]): Student[] => {
        return [...students].sort((a, b) => {
            switch(sortType) {
                ...
                case "rankAsc":
                    return getRankValue(getRankName(a.sum_point ?? 0)) - getRankValue(getRankName(b.sum_point ?? 0));
                case "rankDesc":
                    return getRankValue(getRankName(b.sum_point ?? 0)) - getRankValue(getRankName(a.sum_point ?? 0));
                case "numberAsc":
                    return Number(toHankaku(a.number)) - Number(toHankaku(b.number));
                case "numberDesc":
    ...
    const filteredStudents = selectedClass === "全クラス" 
        ? students 
        : students.filter(student => toHankaku(student.class_id) === toHankaku(selectedClass));
